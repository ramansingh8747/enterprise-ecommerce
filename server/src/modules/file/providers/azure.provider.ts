...
export class AzureBlobStorageProvider implements IStorageProvider {
  ...
  async upload(_file: Buffer | Uint8Array, _metadata: Partial<IFileMetadata>): Promise<IFileMetadata> {
    throw new StorageError('AzureBlobStorageProvider.upload is an architecture contract placeholder.');
  }

  async delete(_pathOrKey: string): Promise<boolean> {
    throw new StorageError('AzureBlobStorageProvider.delete is an architecture contract placeholder.');
  }
  ...
  async getSignedUrl(pathOrKey: string, _expiresInSeconds: number): Promise<string> {
    return `https://blob.core.windows.net/container/${pathOrKey}`;
  }
}
